...
namespace MISA.Meinvoice.Kinesis.Company
{
    public class CompanyRecordProcessor : IShardRecordProcessor
    {
            ...
            foreach (Record rec in records)
            {
                ...
                for (int i = 0; i < NumRetries; ++i)
                {
                    try
                    {
                        ...
                        OrganizationUnit Company = JsonSerializer.Deserialize<OrganizationUnit>(recordData);
                        SyncData(Company);
                        // Uncomment the following if you wish to see the retrieved record data.
                        //object Company = DeserializeFromStream(rec.Data);
                        //Console.Error.WriteLine(
                        //    String.Format("Retrieved record:\n\tpartition key = {0},\n\tsequence number = {1},\n\tdata = {2}",
                        //    rec.PartitionKey, rec.SequenceNumber, JsonSerializer.Serialize(Company)));
        ...
        private void SyncData(OrganizationUnit Company)
        {
            ...
            if (Company != null)
            {
                using (var _connection = new MySqlConnection(configDB))
                {
                    ...
                    try
                    {
                        ...
                        //dynamicParameters.Add("OrganizationUnitCode", customer.OrganizationUnitCode);
                        //dynamicParameters.Add("OrganizationUnitName", customer.OrganizationUnitName);
                        //dynamicParameters.Add("OrganizationUnitType", customer.OrganizationUnitType);
                        //dynamicParameters.Add("ParentID", customer.ParentID);
                        //dynamicParameters.Add("IsParent", customer.IsParent);
                        //dynamicParameters.Add("Description", customer.Description);
                        //dynamicParameters.Add("Inactive", customer.Inactive);
                        //dynamicParameters.Add("IsSystem", customer.IsSystem);
                        _connection.Execute("Proc_SyncCompany", dynamicParameters, commandType: CommandType.StoredProcedure);
                    }
                    catch (Exception ex)
                    {
    ...
    class MainClass
    {
        ...
        public static void Main(string[] args)
        {
            try
            {
                KclProcess.Create(new CompanyRecordProcessor()).Run();
            }
            catch (Exception e)
            {
    ...
    public class OrganizationUnit
    {
        ...
        public string OrganizationUnitSortOrder { get; set; }
        public string OrganizationUnitHRMCode { get; set; }
        public string OrganizationUnitParentName { get; set; }
    }
}
